...
def test_valid_login_logout(test_client, init_database):
    response = test_client.post(
        "/auth/login",
        data={"username": "first_user", "password": "password1"},
...
def test_invalid_login(test_client, init_database):
    response = test_client.post(
        "/auth/login",
        data={"username": "test", "password": "test213"},
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"Login" in response.data
...
def test_valid_register(test_client, init_database):
    ...
    response = test_client.post(
        "/auth/login",
        data={"username": "test_user", "password": "password1"},
        follow_redirects=True,
    )
    assert response.status_code == 200
...
# TODO: add test registration when logged in
# TODO: add check profile paпges
# TODO: add tests task pages
